...
contract StarcoinBridge is IStarcoinBridge, CommitteeUpgradeable, PausableUpgradeable {
    ...
    /// @dev If claimDelaySeconds is 0, the transfer is executed immediately.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        // If no delay configured, execute immediately (backward compatible)
        if (config.claimDelaySeconds() == 0) {
    ...
    /// @notice Legacy function for backward compatibility. Approves and claims in one call if no delay.
    /// @dev Will revert if claimDelaySeconds > 0. Use approveTransferWithSignatures + claimApprovedTransfer instead.
    /// @param signatures The array of signatures.
    /// @param message The BridgeUtils containing the transfer details.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        // This function only works when no delay is configured
        require(config.claimDelaySeconds() == 0, "StarcoinBridge: Use approveTransferWithSignatures when delay is set");
